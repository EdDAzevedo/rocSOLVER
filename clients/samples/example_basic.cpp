...
#define HIP_CHECK(fcn)                                                           \
    {                                                                            \
        ...
        if(istat != hipSuccess)                                                  \
        {                                                                        \
            fprintf(stderr, "HIP error at line %d of %s\n", __LINE__, __FILE__); \
        }                                                                        \
    }
#endif
...
int main()
{
    ...
    double *dA, *dIpiv;
    HIP_CHECK(hipMalloc(&dA, sizeof(double) * size_A));
    HIP_CHECK(hipMalloc(&dIpiv, sizeof(double) * size_piv));
    ...
    HIP_CHECK(hipMemcpy(hA.data(), dA, sizeof(double) * size_A, hipMemcpyDeviceToHost));
    HIP_CHECK(hipMemcpy(hIpiv.data(), dIpiv, sizeof(double) * size_piv, hipMemcpyDeviceToHost));
