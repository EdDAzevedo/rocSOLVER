...
rocsolverStatus_t rf_lusolve(
  ...
  int const m = n;
  {
  bool const isok_arg = (d_LUp != nullptr) &&
                        (d_LUi != nullptr) &&
                        (d_LUx != nullptr) &&
                        (d_b   != nullptr);
  ...
  {
  bool const isok_pointer = ( is_device_pointer(d_LUp) ) &&
                            ( is_device_pointer(d_LUi) ) &&
                            ( is_device_pointer(d_LUx) ) &&
                            ( is_device_pointer(d_b) );
  ...
  HIPSPARSE_CHECK(hipsparseDcsrsv2_bufferSize(
      handle, transL, m, lnnz, descrL, csrSortedValA,
  ...
  stmp = 0;
  HIPSPARSE_CHECK(hipsparseDcsrsv2_bufferSize(
      handle, transU, m, lnnz, descrU, csrSortedValA,
  ...
  HIPSPARSE_CHECK(hipsparseDcsrsv2_analysis(
      handle, transL, m, lnnz, descrL, csrSortedValA, csrSortedRowPtrA,
      csrSortedColIndA, infoL, policy, buffer),
      ROCSOLVER_STATUS_INTERNAL_ERROR);


  HIPSPARSE_CHECK(hipsparseDcsrsv2_analysis(
      handle, transU, m, lnnz, descrU, csrSortedValA, csrSortedRowPtrA,
     ...
     */
        ...
        HIPSPARSE_CHECK(hipsparseDcsrsv2_solve(
                                   handle,
                                   transL,
                                   m,
                                   lnnz,
  ...
  HIPSPARSE_CHECK(hipsparseDestroyMatDescr(descrL),
      ROCSOLVER_STATUS_INTERNAL_ERROR);


  HIPSPARSE_CHECK(hipsparseDestroyMatDescr(descrU),
      ROCSOLVER_STATUS_INTERNAL_ERROR);


  HIP_CHECK(hipFree( (void *) buffer),
            ROCSOLVER_STATUS_ALLOC_ERROR );
  HIP_CHECK(hipFree( (void *) d_y),
            ROCSOLVER_STATUS_ALLOC_ERROR );
